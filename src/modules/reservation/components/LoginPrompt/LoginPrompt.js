...
const LoginPrompt = ({ isMobile }) => {
  ...
  const handleLoginClick = () => {
    navigate(ROUTES.AUTH, { state: { from: ROUTES.RESERVE_TABLE } });
  };

  return (
    <div className={`${styles.loginPromptContainer} ${isMobile ? styles.mobile : ''}`}>
      <h3 className={`${styles.loginTitle} ${isMobile ? styles.mobile : ''}`}>Make a Reservation</h3>
      <p className={`${styles.loginMessage} ${isMobile ? styles.mobile : ''}`}>
        Please login to your account to reserve a table. After logging in, you'll be able to select your preferred date,
        time, and table.
      </p>
      <button
