...
const RestaurantDashboard = () => {
  ...
  const { handleApproveReservation, approveAllReservations, handleRejectReservation } = useReservationManagement(
    pendingReservations,
    setPendingReservations,
    setAllReservations,
    setLoading
  ...
  // Hours management
  const { editingHours, handleHoursChange, handleToggleDay, toggleHoursEditMode, saveHours } = useHoursManagement(
    openingHours,
    setOpeningHours,
    setLoading
  );
  ...
  // Tables management
  const { tablesEditMode, getTableName, toggleTablesEditMode } = useTablesManagement(tables);
  ...
  useWebSocket('wss://rms.bushive.app', activeTab, pendingReservations, setPendingReservations);
        ...
        <main className={`${styles.dashboardContent} ${styles.overflowYAuto} ${styles.p6}`}>
          {loading ? (
            ...
            <>
              ...
              {activeTab === 'tables' && <FloorPlanDesigner editMode={tablesEditMode} />}
              {activeTab === 'employees' && (
                <EmployeeManagement
                  employees={employees}
